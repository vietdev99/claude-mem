...
export class MigrationRunner {
  ...
  runAllMigrations(): void {
    ...
    this.addUserIdToExistingTables();
  }
  ...
  /**
   * Add user_id column to existing tables (migration 21)
   * Links observations, sessions, summaries, and prompts to users (stored in MongoDB)
   */
  private addUserIdToExistingTables(): void {
    const applied = this.db.prepare('SELECT version FROM schema_versions WHERE version = ?').get(21) as SchemaVersion | undefined;
    if (applied) return;

    logger.debug('DB', 'Adding user_id column to existing tables');

    // Helper to add user_id column if it doesn't exist
    const addUserIdColumn = (table: string): boolean => {
      const tableInfo = this.db.query(`PRAGMA table_info(${table})`).all() as TableColumnInfo[];
      const hasUserId = tableInfo.some(col => col.name === 'user_id');

      if (!hasUserId) {
        this.db.run(`ALTER TABLE ${table} ADD COLUMN user_id INTEGER REFERENCES users(id)`);
        this.db.run(`CREATE INDEX IF NOT EXISTS idx_${table}_user_id ON ${table}(user_id)`);
        logger.debug('DB', `Added user_id column to ${table}`);
        return true;
      }
      return false;
    };
    ...
    addUserIdColumn('sdk_sessions');
    addUserIdColumn('observations');
    addUserIdColumn('session_summaries');
    addUserIdColumn('user_prompts');
    addUserIdColumn('pending_messages');

    this.db.prepare('INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)').run(21, new Date().toISOString());

    logger.debug('DB', 'Successfully added user_id columns to existing tables');
  }
}
